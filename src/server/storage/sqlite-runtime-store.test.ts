...
describe("SqliteRuntimeDatabase", () => {
  ...
  it("claims, completes, and replays idempotent responses across database instances", async () => {
    ...
    await expect(first.idempotencyStore.claim(claim)).resolves.toEqual({
      kind: "acquired",
    });
  ...
  it("rejects malformed persisted idempotency responses", async () => {
    ...
    raw.prepare("update idempotency_records set response_value = ? where key_hash = ?").run(
      JSON.stringify({
        status: 201,
  ...
  it("reclaims expired keys without allowing stale claims to complete", async () => {
    ...
    await expect(database.idempotencyStore.claim(first)).resolves.toEqual({
      kind: "acquired",
    });
    await expect(database.idempotencyStore.claim(second)).resolves.toEqual({
      kind: "acquired",
    });
    await expect(
  ...
  it("applies pending runtime migrations to existing local databases", async () => {
    ...
    legacy.prepare("insert into connections (service, connection_name, value, updated_at) values (?, ?, ?, ?)").run(
      "github",
      "default",
      JSON.stringify({
        authType: "api_key",
    ...
    expect(migratedConnection).toMatchObject({
      credential: { apiKey: "legacy-token" },
    });
  ...
  it("rotates stored secret encryption without resetting other runtime data", async () => {
    ...
    await expect(withNewKey.runLogStore.list()).resolves.toMatchObject({
      items: [{ id: "run-1" }],
    });
